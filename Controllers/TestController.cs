...
public class TestController(TmsDbContext context) : ControllerBase
{
    ...
    public async Task<IActionResult> ActiveStudentWithGPA()
    {
        Console.WriteLine("\n>>> STEP 1: Running non-translatable query...");
        try
        {
            var count = await context.Students.Where(s => s.IsActive && s.GPA >= 3.0m).CountAsync();
            return Ok(new { ActiveStudentsGpaCount = count });
        }
        catch (Exception ex)
        {
            Console.WriteLine($">>> EXCEPTION CAUGHT: {ex.Message}\n");
    ...
    [HttpGet("most-enrollments-sorted")]
    public async Task<IActionResult> MostEnrollmentSorted()
    {
        Console.WriteLine("\n>>> STEP 1: Running non-translatable query...");
        try
        {
    ...
    [HttpGet("student-with-zero-enrollment")]
    public async Task<IActionResult> StudentWithZeroEnrollment()
    {
        Console.WriteLine("\n>>> STEP 1: Running non-translatable query...");
        try
        {
